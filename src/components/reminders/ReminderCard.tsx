...
export default function ReminderCard({ birthday, days, reminderDate }: ReminderCardProps) {
  const [month, day] = birthday.date.split("-");
  const dateFormatted = new Date(2000, Number(month) - 1, Number(day)).toLocaleDateString("pt-BR", {
    day: "numeric",
    month: "long",
  });
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          <View>
            <Text style={{ fontSize: 16, fontWeight: "bold" }}>{birthday.name}</Text>
            <Text style={{ fontSize: 12, color: "#6b7280" }}>{dateFormatted}</Text>
          </View>
          ...
          >
            ...
            >
              {days === 0 ? "Hoje" : `${days} ${days === 1 ? "dia" : "dias"} antes`}
            </Text>
          </View>
        </View>
